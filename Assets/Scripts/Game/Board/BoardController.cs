...
public class BoardController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //HelpButton.onClick.AddListener(() => { OnGamePaused(); });
    ...
    private void OnDefeateClicked()
    {
        if (Board.PlayerMode == PlayerMode.Single)
        {
            string message = Board.Map.ActivedCamp == Camp.Wolf ? "你鸽了这场比赛,羊获得胜利!" : "你鸽了这场比赛,狼获得胜利!";
            OnGameOver(message);
        }
        else
        {
            string message = "这场比赛已经没法继续下去了！";
            OnGameOver(message);
        }
    }
    ...
    {
        if (Board.Map.SheepRemaining > 0)
        {
            OnGameMessage("游戏提示", "现在还不是求和的时候！");
        }
        else
        {
            if (Board.PlayerMode == PlayerMode.Single)
            {
                OnGameOver("AI没有拒绝你的请求！");
            }
            else
            {
                OnGameOver("谁也没有赢得这场比赛!");
            }
        }
    }
